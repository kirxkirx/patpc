...
test: patpc
	@echo "Running tests"
	./patpc test_events.txt 2>&1 | grep 'The peak [[:alpha:]]* is at period' | awk '{if ( sqrt( ($$7-5711.5)*($$7-5711.5) ) < 60.0 || sqrt( ($$7-5711.5/2)*($$7-5711.5/2) ) < 30.0 ) print "PASS" ;else print "FAIL("$$7")" }' | { grep "FAIL" && echo "TEST FAIED" && exit 1 || true; }
	...
	./patpc test_events.txt 10000 100 2>&1 | grep 'The peak [[:alpha:]]* is at period' | awk '{if ( sqrt( ($$7-5711.5)*($$7-5711.5) ) < 60.0 ) print "PASS" ;else print "FAIL("$$7")" }' | { grep "FAIL" && echo "TEST FAIED" && exit 1 || true; }
